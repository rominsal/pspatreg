...
Computing average direct, indirect and total marginal effects:
...
As expected, all marginal effects are strongly significant and spillover effects are rather high. We compare these results with those obtained using ML estimates with **`lagsalm()`** (library **`satialreg`**):
...
Now we compute impacts for the SLX model. In this case, contrary to the case of the SAR and SDM models, we do not need simulations to make inference on these marginal effects. We only need to properly compute the variance of total impact using this formula:  
...
Computing average direct and indirect marginal effects:
...
```{r}
spatregsdm <- spatialreg::lagsarlm(formlin, data = ames_sf1,
                                   listw = listW, 
                                   method = "eigen",
                                   Durbin=TRUE) 
W <- as(listW, "CsparseMatrix")
...
# Let's compare direct impacts
round(data.frame(spatialreg_direct=summary(SDM.impact, zstats=TRUE, short=TRUE)$res$direct,sptpreg_direct=summary(eff_parvar_sdm)$dir_table[,1]),3)
# Let's compare indirect impacts
round(data.frame(spatialreg_indirect=summary(SDM.impact, zstats=TRUE, short=TRUE)$res$indirect,sptpreg_indirect=summary(eff_parvar_sdm)$ind_table[,1]),3)
# Let's compare indirect impacts
round(data.frame(spatialreg_indirect=summary(SDM.impact, zstats=TRUE, short=TRUE)$res$total,sptpreg_indirect=summary(eff_parvar_sdm)$tot_table[,1]),3)
```
...
```{r}
linsem <- pspatfit(formlin, data = ames_sf1,
                   listw = listW, 
...
```{r}
spatregsem <- spatialreg::errorsarlm(formlin, data = ames_sf1,
                                     listw = listW, 
                                     method = "eigen") 
summary(spatregsem)
...
round(data.frame(sptpsar=summary(linsem)$bfixed,
                 spatregsem=summary(spatregsem)$coefficients),3)
```
...
The marginal (direct, indirect and total) effects for parametric terms are computed as usual with the function `impactspar`:

```{r}
list_varpar <- c("Fireplaces", "Garage_Cars")
eff_parvar <- impactspar(sp2dsar, list_varpar)
summary(eff_parvar)
```
...
Plot of non-parametric direct, indirect and total effects:
...
Parametric direct, indirect and total effects:

```{r}
list_varpar <- as.character(names(summary(sp2danovasar)$bfixed[1]))
eff_parvar <- impactspar(sp2danovasar, list_varpar)
summary(eff_parvar)
```
...
```{r}
sp2danovasdm <- pspatfit(form2d_psanova, data = ames_sf1, 

                         listw = listW,method = "Chebyshev", 

                         type = "sdm", Durbin = durbinformnonlin, control = list(trace = FALSE)) 
