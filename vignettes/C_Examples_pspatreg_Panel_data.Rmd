...
```{r}
eff_parvar_sar <- impactspar(Linear_WITHIN_sar_REML, listw = lwsp_it)
...
### Parametric and nonparametric direct, indirect and total effects

```{r}
eff_nparvar <- impactspar(sp3danovasarar1, listw = lwsp_it)
summary(eff_nparvar)
```
...
```{r}
par(mar = c(1, 1, 1, 1))
eff_nparvar <- impactsnopar(sp3danovasarar1, listw = lwsp_it, viewplot = TRUE)
```
